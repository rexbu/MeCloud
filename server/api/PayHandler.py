...
from mecloud.lib import alipay_config, alipay_core
from mecloud.lib.WxPaySDK import UnifiedOrder_pub, WxPayConf_pub
        ...
        if good and wallet:
            ...
            if wallet.get('balance', 0) >= good['price'] and chargeFlow:
                ...
                if face:
                    ...
                    uploader_income = good['price'] * withdrawRate * contributorRate * (1 - systemRax)
                    owner_income = good['price'] * withdrawRate * faceOwnerRate * (1 - systemRax)
                    # print 'float uploder_income, owner_income:', uploader_income, owner_income
                    ...
                    uploader_income_show = str(uploader_income)
                    owner_income_show = str(owner_income)
                    # print 'show uploader_income, owner_income:', uploader_income_show, owner_income_show
                    uploader = face.get('uploader', None)
                    owner = None
                    # print 'face:', face
                    # print 'face.get assign:', face.get('assign', None)
                    # print 'face[assign].get(user):', face['assign'].get('user', None)
                    if face.get('assign', None) and face['assign'].get('user', None):
                        owner = face['assign']['user']
                    print 'owner:%s', owner
                    print 'before update db, uploder_income, owner_income:', uploader_income, owner_income
                    if uploader:
                        uploader_wallet_action = {'destClass': 'Wallet', 'query': {'user': face['uploader']},
                                                  'action': {'@inc': {'income': uploader_income}}}
                    if owner:
                    ...
                    chargeFlow_action = {'destClass': 'ChargeFlow', 'query': {'_id': chargeFlow['_id']},
                                         'action': {'@set': {'status': 1}}}
                    actions = [chargeFlow_action]
                    if uploader:
                        actions.append(uploader_wallet_action)
                    if owner:
                        actions.append(owner_wallet_action)
                    # 计入收益记录
                    ...
                    if uploader:
                        ...
                        uploader_income_flow['income'] = uploader_income
                        ...
                        uploader_income_flow['media'] = face['file']

...
class AlipayCallbackHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        logger.debug('check_res:%s', check_res)
        if check_res == False:
            logger.warn('验签失败,非法订单')
            self.write("fail")
            return

        # 这里是去访问支付宝来验证订单是否正常
        res = verify_from_gateway({"partner": alipay_config.partner_id, "notify_id": params["notify_id"]})
        logger.debug('res from alipay:%s', res)
        if res == False:
            logger.warn('查询支付宝订单状态为false, 非法订单')
            self.write("fail")
            return

        trade_status = params["trade_status"]
        logger.debug('trade_status:%s', trade_status)
        """
        下面是处理付款完成的逻辑
        """
